"""MNC Audio Copier v1.0 - PyQt6 GUI"""
...
from .config import APP_NAME, APP_VERSION, DEFAULT_SOURCE_BASE, DEFAULT_OUTPUT_BASE, WEEKDAY_KR
from .scanner import CopySession, scan_sessions
from .copier import copy_session
...
class CopyThread(QThread):
    ...
    def __init__(self, sessions: List[CopySession], output_base: Path):
        super().__init__()
    ...
    def run(self):
        ...
        for idx, session in enumerate(self.sessions):
            ...
            self.progress.emit(
                idx + 1, total_sessions,
                f"복사 중: {session.location}/{session.point}"
            )
            ...
            if result.success:
                self.session_complete.emit(
                    idx, True, f"{result.files_copied}개 파일 복사"
                )
                success_count += 1
            else:
                self.session_complete.emit(
                    idx, False, ", ".join(result.errors[:3])
                )
                fail_count += 1
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.sessions: List[CopySession] = []
        self.copy_thread: Optional[CopyThread] = None
    ...
    def _init_ui(self):
        ...
        # 입력 섹션
        input_group = QGroupBox("소스 설정")
        input_layout = QVBoxLayout(input_group)

        source_layout = QHBoxLayout()
        source_layout.addWidget(QLabel("소스 경로:"))
        self.source_edit = QLineEdit()
        self.source_edit.setText(str(DEFAULT_SOURCE_BASE))
    ...
    def _browse_source(self):
        ...
        path = QFileDialog.getExistingDirectory(
            self, "소스 폴더 선택", self.source_edit.text()
        )
        if path:
    ...
    def _scan_sessions(self):
        ...
        self._log("스캔 시작...")
        self.sessions = scan_sessions(source_path)
    ...
    def _populate_table(self):
        ...
        for row, session in enumerate(self.sessions):
            ...
            # 위치
            loc_item = QTableWidgetItem(session.location)
            loc_item.setData(Qt.ItemDataRole.UserRole, row)
            self.session_table.setItem(row, 1, loc_item)

            # 지점
            point_item = QTableWidgetItem(session.point)
            point_item.setData(Qt.ItemDataRole.UserRole, row)
            self.session_table.setItem(row, 2, point_item)
            ...
            # 요일
            weekday_item = QTableWidgetItem(session.weekday)
            weekday_item.setData(Qt.ItemDataRole.UserRole, row)
            self.session_table.setItem(row, 4, weekday_item)

            # 파일수
            count_item = QTableWidgetItem(str(session.wav_count))
            count_item.setData(Qt.ItemDataRole.UserRole, row)
            self.session_table.setItem(row, 5, count_item)
    ...
    def _get_selected_sessions(self) -> List[CopySession]:
        """선택된 세션 반환"""
        selected = []
        for row in range(self.session_table.rowCount()):
            item = self.session_table.item(row, 0)
    ...
    def _start_copy(self):
        ...
        self._log(f"복사 시작: {len(selected)}개 세션")

        self.copy_thread = CopyThread(selected, output_path)
        self.copy_thread.progress.connect(self._on_progress)
        self.copy_thread.session_complete.connect(self._on_session_complete)
        self.copy_thread.finished_all.connect(self._on_finished)
